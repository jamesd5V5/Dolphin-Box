...
def classify_wav(filepath, temperature):
    ...
    return predictions, avg_probs, segment_probs
    ...
    for temperature in TEMPERATURES:
        ...
        for filename in os.listdir(TRACK_FILES):
            if filename.endswith('.wav'):
                ...
                print(f"\n{filename}==================================")
                preds, avg_probs, segment_probs = classify_wav(filepath, temperature)
                ...
                all_segment_probs.extend(segment_probs)
        
        results[temperature] = file_results
    
    import json
    with open('results.json', 'w') as f:
        json.dump(all_segment_probs, f)
